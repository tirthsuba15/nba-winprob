...
| xgboost | 0.7087 | 0.2170 | 70.2% | all features |

Lower log-loss and Brier = better; we care most about calibration.

Numbers from a real backtest: train on 2023-24 (146,008 moments), test on 2024-25
(147,878 moments). Generated by `python src/model.py --data data --test-seasons 2024-25`.

> **Honest note:** XGBoost's log-loss (0.7087) is *worse* than naive (0.6868). This is not
> a bug — it reflects overconfidence at the extremes: the model pushes probabilities near 0
> or 1 too aggressively, and log-loss punishes wrong confident predictions hard. Logistic
> regression, which can't extrapolate as wildly, is the best model here. The next step is
> applying a calibration wrapper (sklearn's `CalibratedClassifierCV`) before declaring
> XGBoost the winner.
