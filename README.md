...
|---|---|---|---|
| naive (season avg to date) | 4.75 | — | — |
| xgboost (mean + 80% interval) | 4.55 | 80.1% | 14.5 pts |
...
**Features** (importance, points model): `usage_roll5` (0.36), `pts_roll10`
(0.34), `pts_season_avg` (0.20), then `pred_minutes` from a **minutes sub-model**
and `top2_teammate_out` from the leakage-safe teammate-availability proxy. The
minutes sub-model is a separate XGBoost regressor predicting minutes from
`[min_season_avg, home, days_rest, b2b, top2_teammate_out]`; the points model
uses its `pred_minutes` output **instead of** a raw minutes average. Training
also applies **exponential recency weighting** (`weight = exp(-0.1 · games_ago)`,
as XGBoost `sample_weight`). Recent-form features `pts_last3_avg` and
`pts_vs_opponent_hist` (career avg vs that opponent) add small lift. Opponent
pace / defensive rating / defense-vs-position and the
pregame blowout proxy each contribute ~0.008 — small, but included and reported
honestly. Interval coverage is restored to **80.1%** (quantile alphas tuned to
0.085 / 0.915 after exponential recency weighting had narrowed the interval).
...
|---|---|---|
| top 5% most-confident | 5% | **0.764** |
| top 10% | 10% | 0.718 |
| top 25% | 25% | 0.664 |
| top 50% | 50% | 0.625 |
| **all calls** | 100% | **0.576** |

The top-5% has plateaued at ~0.76 across six feature additions — the honest
ceiling for a model whose "line" is the player's own season average.
